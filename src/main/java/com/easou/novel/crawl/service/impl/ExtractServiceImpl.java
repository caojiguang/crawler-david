...
//		template = templateFactory.getTemplate(catalogUrl.getEntryWay());
...
//				System.out.println(url + "\t" + template.getContentRegexs());
	...
	public CrawlBasicInfo extractHtmlContent(CrawlUrl contentUrl) {
		...
		StopWatch stopWatch = new Log4JStopWatch();
		Template template = null;
//		template = templateFactory.getTemplate(contentUrl.getEntryWay());
		...
		if(contentTemplate != null) {
			boolean res = mergePageContent(contentUrl, template, contentTemplate, document, crawlBasicInfo);//迭代做合页操作
			if (!res) {
				return new CrawlBasicInfo();
			}
		}
...
//		template = templateFactory.getTemplate(contentUrl.getEntryWay());
	...
	private List<CrawlImage> transformImages(Elements elements, String entryWay) {
        ...
        for (Element element : elements) {
			...
			byte[] data = httpClientService.getImageByGet(url);
			if(data == null){
	...
	private boolean mergePageContent(CrawlUrl contentUrl, Template template, TemplateBasicInfo contentTemplate, Document document, CrawlBasicInfo crawlBasicInfo) {
		...
		return mergePageContent(contentUrl, template, contentTemplate, document, crawlBasicInfo);
	}
