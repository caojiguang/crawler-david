package com.easou.novel.crawl.task;
...
import com.easou.novel.crawl.result.ResultImpl;
		...
		while(true){
			...
			try {
				...
				if(crawlUrl.getType() == 0){
					...
					StopWatch stopWatch = new Log4JStopWatch();
					contextBase.put(CommandContant.PREPARE_CRAWL_URL, crawlUrl);
					logger.info(crawlUrl.getUri().toString() + " pop crawlUrl: type[{}] rankOrder[{}]", crawlUrl.getType(), crawlUrl.getRankOrder() );
